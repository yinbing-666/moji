...
- [x] 提交 v0.2.1 修复：`c511d15`；安装测试阻塞修复：`5752869`。
...
- **服务检查**：覆盖安装后的主程序从 `D:\Desktop\墨记\activitywatch\aw-server-rust.exe` 启动内置服务，`127.0.0.1:5601` 返回 `moji / v0.13.2`；终止初始子进程后，守护线程约 2 秒内启动新进程并恢复健康响应。
...
- **Git 状态**：v0.2.1 修复基线为 `c511d15`，安装测试阻塞修复为 `5752869`；覆盖安装验收记录已本地提交，尚未推送、打标签或创建 GitHub Release。
