...
- **Rust 测试**：`cargo test --locked` 结果 21 passed，0 failed，6 ignored；覆盖报告历史增量迁移、报告元数据加密同步、旧快照兼容、FTS、本地 API、MCP 路径选择、ActivityWatch 接管决策、汇总、PDF 文本处理、数据清理和周计划持久化。
...
- **发布校验**：`scripts/check-release.ps1` 通过，六处版本均为 `0.2.1`；NSIS 安装包 `墨记_0.2.1_x64-setup.exe` 为 12,276,841 字节，版本元数据为 `0.2.1`，SHA-256 为 `d20f6c1f0920f3c7f35865b757e79b97d3d173a19301a8b8c271aeebdc137989`，当前未签名。
- **服务检查**：内置 ActivityWatch 二进制使用隔离数据库在 `127.0.0.1:5611` 返回 v0.13.2；守护决策测试覆盖复用现有服务、服务消失后接管和自有子进程运行时避免重复启动。
...
- **release MCP**：独立 `moji-mcp.exe` 使用隔离测试库返回 MCP 协议 `2025-06-18`、版本 `0.2.1` 和 3 个只读工具；健康检查、搜索与汇总均成功，汇总活动时长为 600 秒。
- **真实回溯**：桌面 APP 使用 SQLite 返回 6 条本周活动，搜索 `ChatGPT` 返回 26 条匹配记录。
- **Git 状态**：v0.2.1 修复基线为 `c511d15`；安装测试阻塞修复已完成并本地提交，尚未推送、打标签或创建 GitHub Release。
