  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-h3 font-semibold text-ink">采集链路</h2>
          <p className="mt-1 text-xs text-ink-muted">从窗口读取到本地落盘的实时状态，不包含任何云端服务。</p>
        </div>
        <div className="flex items-center gap-2">
          <button type="button" onClick={onCaptureNow} disabled={isCapturing} className="rounded-md border border-line-strong bg-surface px-3 py-1.5 text-xs font-medium text-ink-muted hover:border-accent disabled:opacity-50">立即采集</button>
          <button type="button" onClick={() => void refresh()} disabled={checking} className="rounded-md bg-inverse px-3 py-1.5 text-xs font-medium text-on-inverse hover:bg-inverse-hover disabled:opacity-50">{checking ? '检测中…' : '重新检测'}</button>
        </div>
      </div>

      <div className="divide-y divide-line-soft rounded-lg border border-line bg-surface px-5">
        {checks.map(check => (
          <div key={check.label} className="grid gap-1 py-4 sm:grid-cols-[9rem_minmax(0,1fr)] sm:gap-4">
            ...
            <div className="min-w-0">
              <p className="text-sm text-ink-muted">{check.value}</p>
              <p className="mt-0.5 truncate text-xs text-ink-faint" title={check.detail}>{check.detail}</p>
            </div>
          </div>
        ))}
      </div>

      <section className="border-l-3 border-accent bg-accent-soft px-4 py-3">
        <p className="text-sm font-medium text-ink">当前隐私模式：{settings.dataSource === 'local' ? '无 LLM' : '有 LLM'}</p>
        <p className="mt-1 text-xs leading-5 text-ink-muted">{settings.dataSource === 'local' ? '窗口信息只在本机经过分类规则处理，不调用外部模型。' : '仅在模型配置完整且采集开启后，将允许的窗口文本发送到用户配置的接口。'}</p>
      </section>

      <p className="text-right text-[11px] text-ink-faint">{checkedAt ? `最后检测：${checkedAt.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}` : '尚未检测'}</p>
    </div>
  )
}
